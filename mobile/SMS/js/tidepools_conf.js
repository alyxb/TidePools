...
function landmarkSubmitter(){
	...
	var map = new L.Map('map', {crs:L.CRS.EPSG3857});
		var treekitUrl = '1.0.0/conftest/{z}/{x}/{y}.png',
		treekit = new L.TileLayer(treekitUrl, {minZoom: 14, maxZoom: 18, scheme:"tms"});  //this is the layer!! the tile map
		
		
		
		map.setView(new L.LatLng(42.36219069106654,-83.06988000869751), 16).addLayer(treekit); //setting current view to map layer
...
*/
		...
		treekit = new L.TileLayer(treekitUrl, {minZoom: 16, maxZoom: 20, scheme:"tms"});  //this is the layer!! the tile map
...
*/
		...
		treekit = new L.TileLayer(treekitUrl, {minZoom: 3, maxZoom: 20});  //this is the layer!! the tile map
		
		
		map.setView(new L.LatLng(40.67645788914882,-74.00529831647873), 13).addLayer(treekit); //setting current view to map layer
	...
	var lastZoom = 12; //this is the zoom the map starts with right now
	 	...
	 	drop = new L.Marker(new L.LatLng(lat, lng), {icon: current});
	   	...
	   	drop.on('dragend',resendCoord);
			...
			var eventpopup = 
				...
				+'</br><hr>'
				+'</br><b>When is it? (landmark will appear/disappear during this time period) </b></br>'
				...
				+'</br>Start Date/Time  <input type="text" name="startdatetimepicker" id="startdatetimepicker" value="Click Here" />'
				+'</br>End Date/Time  <input type="text" name="enddatetimepicker" id="enddatetimepicker" value="Click Here" />'
			...
			var grouppopup = 
				...
				+'<form id="newPost" action="php/record_landmark.php" method="post" onsubmit="this.submit(); return false;">'
				+'</select>'
				+'Title/Name</br><input type="text" id="name" name="name" maxlength="50" value=""/></br>'
				+'Anything else?</br> <textarea name="description" id="description" maxlength="300"></textarea></br>'
				+'<input type="button" id="submit" value="Save" onsubmit="this.submit(); " />'
				+'<input type="button" id="submit" value="Cancel" onClick="window.location.reload()" />'	
				+'</br><hr>'
